...
if args.database_index_configuration == None:
    ...
    "preprocessings":{
        "pipeline":[["drop_channels", {'ch_names': ["ECG EKG", "Manual"], 'on_missing': 'warn'}], ["asr", {"cutoff": 30}], ["average_reference", {}], ["min_max_nor", {}]]
    }
    }
else:
...
for person_index in record_indexes:
    ...
    if person_index not in load_preprocessing:
        ...
        if save_preprocessed_data:
            mne.export.export_raw(os.path.join(store_base_path, f'[preprocessed_asr]p{person_index}.edf'), data, overwrite=True)
    else:
        print(f"Load preprocessed data...")
        data = mne.io.read_raw(os.path.join(store_base_path, f'[preprocessed_asr]person{person_index}.edf'))
    ...
    for n_states in range(microstate_search_range[0], microstate_search_range[1]):
        ...
        if n_states == 4 and store_4_microstates:
            store(recording.latent_maps, recording.latent_segmentation, recording.gev_tot, "[asr]", person_index)
